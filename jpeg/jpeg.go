...
void init(){
	void *lib_handle;
	lib_handle = dlopen("/opt/homebrew/lib/libjpeg.8.2.2.dylib", RTLD_NOW);
	if (!lib_handle) {
		fprintf(stderr, "%s\n", dlerror());
		exit(1);
	}
	...
	//dlerror();
	//void *function = dlsym(lib_handle, "jpeg_CreateDecompress");
	//char *error = dlerror();
	//if (error != NULL) {
	//	  fprintf(stderr, "Error while looking up symbol %s: %s\n", "jpeg_CreateDecompress", error);
	//	  return;
	//  }
	populate_symbols_table_from_handle(lib_handle, &symbols_table);
}
